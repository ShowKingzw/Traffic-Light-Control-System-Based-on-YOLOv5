        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for c in det[:, 5].unique():
                    ...
                    total_count += n            # 统计所有结果
                    s += f"{n} {names[int(c)]}{'s' * (n > 1)}, "  # add to string
                maxGoal = getMaxNum(maxGoal,total_count) # 返回最大目标数
                print("最后的结果是 : {} 辆车".format(total_count))
...
def parse_opt():
    ...
    parser.add_argument('--classes', nargs='+', type=int,default=[1,2,3,4,5,6,7], help='filter by class: --classes 0, or --classes 0 2 3')
...
def main(opt):
    detectImgPath = ROOT / 'data/images'
    check_requirements(ROOT / 'requirements.txt', exclude=('tensorboard', 'thop'))
    max = run(**vars(opt)) 
    # print(int(max)) 
    # deleteImg(detectImgPath)
    return max

def deleteImg(folder_path):
    # 遍历文件夹中的所有文件和子文件夹
...
def getMaxNum(previous,reslut):
    if previous == 0:
        return reslut
    else:
        return max(reslut,previous)
